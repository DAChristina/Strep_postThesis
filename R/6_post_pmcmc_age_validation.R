...
age_validation <- function(n_sts){
  ...
  incidence_modelled <- dplyr::bind_rows(
    read.csv(paste0(dir_name, "incidence_modelled_GPSC55.csv")) %>% 
  ...
  serotype12F_data <- read.csv("inputs/12F_Jan_2025_combined_cleaned.csv") %>% 
    glimpse()
  
  GPSC55_data <- read.csv("inputs/genomic_data_cleaned.csv") %>% 
