...
pars <- list()
n_particles <- 1L
n_times <- 5112 # roughly from 2010 to 2023 in days
...
for (i in particles){
  ...
  pars[[i]] <- list(
    ...
    hypo_sigma_2 = 1,
    log_delta1 = as.numeric(results[5, i]),
    log_delta2 = as.numeric(results[6, i])
  )
...
png(paste0(dir_name, "figs/model_vs_data_withCrI_and10Particles_allAge.png"),
...
# Age group 0-44
pdat_D1 <- data_storage %>%
  dplyr::filter(compartment %in% c("model_D1",
                                   "data_count_55_1",
                                   "data_count_12F_1"
  )) %>%
  tidyr::pivot_wider(
  ...
  dplyr::select(-contains("particle_")) %>% 
  tidyr::unnest(cols = c(parameter_values,
                         data_count_55_1,
                         data_count_12F_1
  )) %>%
  dplyr::rename(model_D1 = parameter_values) %>%
  dplyr::mutate(model_D1_low = qnbinom(0.025,
                                       size = results[7,4],
                                       mu = model_D1),
  ...
  # ) %>%
  tidyr::complete(yearWeek,
                  fill = list(data_count_55_1 = 0,
                              data_count_12F_1 = 0)) %>% 
  glimpse()

# write.csv(pdat_D1,
#           paste0(dir_name,
#                  "incidence_modelled_serotype1_with_top10_particles_ageGroup1_044.csv"),
#           row.names = F)

p_D1 <- ggplot(pdat_D1, aes(x = yearWeek)) +
...
# Age group 45+
pdat_D2 <- data_storage %>%
  dplyr::filter(compartment %in% c("model_D2",
                                   "data_count_55_2",
                                   "data_count_12F_2"
  )) %>%
  tidyr::pivot_wider(
  ...
  dplyr::rename(model_D2 = parameter_values) %>%
  dplyr::mutate(model_D2_low = qnbinom(0.025,
                                       size = results[7,4],
                                       mu = model_D2),
  ...
  # ) %>%
  tidyr::complete(yearWeek,
                  fill = list(data_count_55_2 = 0,
                              data_count_12F_2 = 0)) %>% 
  glimpse()

# write.csv(pdat_D2,
#           paste0(dir_name,
#                  "incidence_modelled_serotype1_with_top10_particles_ageGroup2_45plus.csv"),
#           row.names = F)

p_D2 <- ggplot(pdat_D2, aes(x = yearWeek)) +
