# highest likelihood from tuned_mcmc2 instead
...
pmcmc_samples <- read.csv(paste0(dir_name,
                                "mcmc2_burnedin.csv"))

top_idx <- which.max(pmcmc_samples$log_likelihood)
top_pars <- pmcmc_samples[top_idx,]
...
pars <- list(
  N_ini = 6.7e7,
  log_A_ini = as.numeric(top_pars[1,"log_A_ini"]),
  D_ini = 0,
  R_ini = 0,
  time_shift_1 = as.numeric(top_pars[1,"time_shift_1"]),
  beta_0 = as.numeric(top_pars[1,"beta_0"]),
  beta_1 = as.numeric(top_pars[1,"beta_1"]),
  hypo_sigma_2 = 1,
  log_delta1 = as.numeric(top_pars[1,"log_delta1"]),
  log_delta2 = as.numeric(top_pars[1,"log_delta2"])
)
...
pdat <- incidence_modelled %>%
  ...
  tidyr::unnest(cols = c(model_D, model_D1, model_D2,
                         ...
                         data_count_12F_2,
                         data_count_12F_all)) %>% 
  dplyr::mutate(data_count_55_1 = round(data_count_55_1, 0),
                data_count_55_2 = round(data_count_55_2, 0),
                data_count_55_all = round(data_count_55_all, 0),
                kappa = top_pars[1,"kappa_55"],
                model_D_low = qnbinom(0.025,
                                      size = kappa,
                                      mu = pmax(model_D, 1e-8)), # avoid zeros
                model_D_high = qnbinom(0.975,
                                       size = kappa,
                                       mu = pmax(model_D, 1e-8)),
                model_D1_low = qnbinom(0.025,
                                       size = kappa,
                                       mu = pmax(model_D1, 1e-8)),
                model_D1_high = qnbinom(0.975,
                                        size = kappa,
                                        mu = pmax(model_D1, 1e-8)),
                model_D2_low = qnbinom(0.025,
                                       size = kappa,
                                       mu = pmax(model_D2, 1e-8)),
                model_D2_high = qnbinom(0.975,
                                        size = kappa,
                                        mu = pmax(model_D2, 1e-8)),
                logLik_D1 = ifelse(dnbinom(data_count_55_1, size = kappa,
                                    mu = pmax(model_D1, 1e-8), log = TRUE) == "-Inf", -1e10,
                                   dnbinom(data_count_55_1, size = kappa,
                                           mu = pmax(model_D1, 1e-8), log = TRUE)),
                logLik_D2 = ifelse(dnbinom(data_count_55_2, size = kappa,
                                    mu = pmax(model_D2, 1e-8), log = TRUE) == "-Inf", -1e10,
                                   dnbinom(data_count_55_1, size = kappa,
                                           mu = pmax(model_D1, 1e-8), log = TRUE)),
                logLik_D = logLik_D1 + logLik_D2,
                across(starts_with("logLik_"), ~ replace_na(., 0))
...
write.csv(pdat,
          paste0(dir_name,
                 "incidence_modelled_GPSC55_with_top_particles_ageGroup2_45plus_withKappa.csv"),
          row.names = F)


write.csv(top_pars,
          paste0(dir_name,
                 "highest_loglikelihood_particle_from_tunedmcmc2.csv"),
          row.names = F)
