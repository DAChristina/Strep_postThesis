...
pars <- list(m = t_norm,
             ...
             log_I_ini = c(0.35, 0.3), # test c(0.65, 0.35),
             time_shift_1 = 0.1254,
             beta_0 = 0.5,
             beta_1 = 0.3
)
...
incidence_modelled <- 
  ...
  dplyr::mutate(compartment = 
                  dplyr::case_when(index == 1 ~ "Time",
                                   ...
                                   index == 4 ~ "total I",
                                   index == 5 ~ "total R",
                                   index == 6 ~ "n_SI1_weekly",
                                   index == 7 ~ "n_SI2_weekly",
...
p1 <- ggplot(incidence_modelled %>% 
               dplyr::filter(
                 compartment %in% c("model_D1", "n_SI1_weekly",
                                    "data_count_s1_1"),
                 compartment != "Time",
               )
             ,
...
p2 <- ggplot(incidence_modelled %>% 
               dplyr::filter(
                 compartment %in% c("model_D2", "n_SI2_weekly",
                                    "data_count_s1_2"),
                 compartment != "Time",
               )
             ,
